...
public class CategoryHandler (AppDbContext context) : ICategoryHandler
{
    ...
    public async Task<Response<Category?>> DeleteAsync(DeleteCategoryRequest request)
    {
        ...
        {
            return new Response<Category?>(null, 500, "Categoria não foi excluída");
        }
    }
    ...
    public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
    {
        ...
        {
            return new PagedResponse<List<Category>?>(null, 500, "As categorias não foram encontradas");
        }

    }


    public async Task<Response<Category?>> GetByIdAsync(GetCategoryByldRequest request)
    {
        try
        {
            var category = await context
                .Categories
                .AsNoTracking() // Get muito mais rápido
